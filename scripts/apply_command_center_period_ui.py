...
api = replace_between(
  ...
  factory CommandCenterData.fromJson(Map<String, dynamic> json) =>
      CommandCenterData(
        ...
        period: Map<String, dynamic>.from(
          json['period'] as Map? ?? const {},
        ),
...
old_get = '''  Future<CommandCenterData> getCommandCenter() async =>
...
'''
new_get = '''  Future<CommandCenterData> getCommandCenter({
    DateTime? from,
    DateTime? to,
    String mode = 'day',
  }) async {
...
'''
if old_get not in api:
    raise SystemExit('getCommandCenter block not found')
api = api.replace(old_get, new_get)
api_path.write_text(api, encoding='utf-8')

# ---- Sejarah Rondaan: accept deep-link date/department/status -----------------
...
'''  late DateTime _selectedDate;
  ...
  void initState() {
    ...
    _selectedDepartmentId = widget.initialDepartmentId ?? widget.user.departmentId;
                      ...
                      Wrap(
                        ...
                        children: [
                          ...
                          ChoiceChip(
                            ...
                            onSelected: (_) => setState(() => _filter = 'complete'),
                          ),
                        ],
                      ),
'''
if needle not in history:
    raise SystemExit('history date filter insertion point not found')
...
'''                  final history = snapshot.data!;
                  ...
                  return ListView(
''')
history = history.replace("Text('${history.patrolRuns.length} sesi'),", "Text('${visibleRuns.length} sesi'),")
history = history.replace('if (history.patrolRuns.isEmpty)', 'if (visibleRuns.isEmpty)')
history = history.replace('...history.patrolRuns.map(', '...visibleRuns.map(')
history = history.replace(
...
# ---- Sejarah Kehadiran: accept deep-link date ---------------------------------
attendance_path = root / 'lib/features/admin/attendance_history_screen.dart'
...
# ---- New Pemantauan Rondaan UI ------------------------------------------------
command_path = root / 'lib/features/admin/command_center_screen.dart'
command_path.write_text(r'''import 'dart:async';
import 'dart:convert';
...
class _CommandCenterScreenState extends State<CommandCenterScreen> {
  ...
  Future<void> _openHistory({
    ...
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Sesi telah tamat. Sila log masuk semula.')),
      );
      return;
    }
  ...
  void _openAttendance() {
    Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => AttendanceHistoryScreen(api: widget.api, initialDate: _to),
      ),
    );
  }
      ...
      builder: (sheetContext) => _DetailSheet(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final date = DateTime.tryParse(row['lastScanAt'] as String? ?? '')
                    ?.toLocal() ??
                _to;
            return ListTile(
  ...
  Future<void> _showCoverageDays({
    ...
    final rows = source
        .where(
          (row) => !onlyComplete || ((row['completeSessions'] as num?)?.toInt() ?? 0) > 0,
        )
        .toList()
      ...
      builder: (sheetContext) => _DetailSheet(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            final row = rows[index];
            final date = _dateFromKey(row['date']) ?? _to;
            final missing = (row['missingCheckpoints'] as List<dynamic>? ?? const [])
                .map((item) => Map<String, dynamic>.from(item as Map)['name'])
                .join(', ');
            return Card(
  ...
  Future<void> _updateIncident(int id, String status) async {
    try {
      ...
      if (!mounted) return;
      _message('Status insiden ditukar kepada ${_incidentStatusLabel(status)}.');
    } catch (error) {
      if (!mounted) return;
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _PatrolCoverageOverview(
                      summary: summary,
                      periodLabel: _periodLabel(),
                      onScanned: () => _showCoverageDays(title: 'Checkpoint Diimbas'),
                    ...
                    _SectionHeader(
                      ...
                      subtitle: '${guards.length} pengawal mempunyai rekod scan',
                    ...
                    KeyedSubtree(
                      ...
                      child: _SectionHeader(
                        ...
                        subtitle: '${incidents.length} rekod • $openIncidents masih terbuka',
                      ),
                    ),
                    const SizedBox(height: 10),
                    if (incidents.isEmpty)
                      ...
                      ...incidents.take(20).map(
                            (incident) => _IncidentCard(
                              ...
                              onImages:
                                  ((incident['image_count'] as num?)?.toInt() ?? 0) == 0
                                      ? null
...
# Version bump for the production release.
pubspec_path = root / 'pubspec.yaml'
pubspec = pubspec_path.read_text(encoding='utf-8')
if 'version: 0.5.8+24' in pubspec:
    pubspec = pubspec.replace('version: 0.5.8+24', 'version: 0.5.9+25', 1)
pubspec_path.write_text(pubspec, encoding='utf-8')

print('Applied period-aware Pemantauan Rondaan UI and deep links.')
